...
const resolvers = {
  ...
  Mutation: {
    ...
    async upsertTrendLayout(parent, {trendLayout}, {dataSources}, context) {
      return await dataSources.trendLayouts.upsert( {name: trendLayout.name, trendId: trendLayout.trendId, username: trendLayout.username}, trendLayout);
    },
...
class MongoDataSourceExtended extends MongoDataSource {
  ...
  async upsert(query, data) {
    return await db.upsert(this.model, query, data);
  }
...
class TrendGraphDataSource extends MongoDataSourceExtended {
  ...
  async upsertUniqueXValue(query) {
    ...
    const ret = await db.upsert(this.model, queryOnly, data);
